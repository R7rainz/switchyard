...
// behave the same way about workspace scoping and replacement.
...
func (m *MemoryStore) key(workspaceID, provider, name string) string {
	return string(aad(workspaceID, provider, name))
}
...
func (m *MemoryStore) Save(_ context.Context, record Record) error {
	...
	key := m.key(record.WorkspaceID, record.Provider, record.Name)
...
func (m *MemoryStore) Get(_ context.Context, workspaceID, provider, name string) (Record, error) {
	m.mu.Lock()
	defer m.mu.Unlock()

	record, ok := m.records[m.key(workspaceID, provider, name)]
	if !ok {
...
func (m *MemoryStore) List(_ context.Context, workspaceID string) ([]Record, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MemoryStore) Delete(_ context.Context, workspaceID, provider, name string) error {
	m.mu.Lock()
	defer m.mu.Unlock()

	key := m.key(workspaceID, provider, name)
...
func sortRecords(records []Record) {
	slices.SortFunc(records, func(a, b Record) int {
		return strings.Compare(
			a.WorkspaceID+"\x00"+a.Provider+"\x00"+a.Name,
			b.WorkspaceID+"\x00"+b.Provider+"\x00"+b.Name,
		)
	})
}
